...
<p class="warning">URL syntax below uses Markdown format (<code><span>[example_link](https://example.com)</span></code>) & won't open in new tab! <b>Be sure to always use Middle Click if you like to make sure staying here.</b></p>
## Info:
### Threads
...
- [Prepare QEMU](https://foss.heptapod.net/magic-lantern/magic-lantern/-/blob/branch/qemu/contrib/qemu/README.rst ).
    ...
    - [pls clean up & redo exactly said](https://www.magiclantern.fm/forum/index.php?topic=20214.msg241010#msg241010 ). give last source that causes problem the sha1sum. [here's how](https://askubuntu.com/a/61828/64076)
    - Same Failed! [log](/assets/txt/logs/attempt_ml_2022-11_23/clear_command_fail.log) & sha1sum of `vpc.c` in `qemu-eos` repo is `f9ad389e82886eb53f242bde967519fac416e891`.
    - [Suggested that GCC version too high](https://www.magiclantern.fm/forum/index.php?topic=20214.msg241013#msg241013). my GCC is `11.3.0` (use `gcc --version`) right now! [log](/assets/txt/logs/attempt_ml_2022-11-24/downgrade_gcc.log)
    - Okay, package `gcc-10` exist. and `./configure` has a way to choose older GCC. [here's how](https://stackoverflow.com/a/3698487/9079640 ).
    - ah crack. [log](/assets/txt/logs/attempt_ml_2022-11-24/retry_lan.log)
    - [Somehow GCC still stuck at here `11`](https://www.magiclantern.fm/forum/index.php?topic=20214.msg241019#msg241019 ). Sigh! you know what, let's break my WSL. we're gonna make the gcc defaults to `10`. [Found way!](https://askubuntu.com/a/26518/64076) Replace the `4.X.X` thingy with `10`!!
    - [fail](/assets/txt/logs/attempt_ml_2022-11-24/still_somehow_same_error_despite.log)
    - Okay, go back to GCC 9? [Fail different error](/assets/txt/logs/attempt_ml_2022-11-24/dedd_oops.log).
    - Maybe I forgot to last step in that stack overflow? clear alternatives & update alternatives to `10` again, **do not forget configure that update alternatives!**. okay nothing happens because I only have installed 1 alternative for each.
    - try again. [configure with `--disable-werror`](https://gitlab.com/qemu-project/qemu/-/issues/1064 ). same `-nopie` mistake error. [log](/assets/txt/logs/attempt_ml_2022-11-24/eyap_error_locks_to_different.log)
    - [Ubuntu horribly fails!](https://www.magiclantern.fm/forum/index.php?topic=20214.msg241024#msg241024) Gave up! IDK HOW TO GCC!!! Just `iso` the Debian 11 Bullseye instead! [**AAAAAAAAAAAAAAAAAAAAAAAAAAAA**](https://www.debian.org/download)
    - Virtualbox setup! [install essential](/assets/txt/logs/attempt_ml_2022-11-25/install_essentials.log).
    - [idk, sort of working](/assets/txt/logs/attempt_ml_2022-11-25/install_works_sort_of.log).
    - [Python missing vncdotool](/assets/txt/logs/attempt_ml_2022-11-25/vncdotool_calamity.log) but fortunately I just have to `pip intall vncdotool` wtf!??!?!
    - [**QEMU COMPILE WORKS!!!**....](/assets/txt/logs/attempt_ml_2022-11-25/finally_qemu_sort_of_work.log) right? I need the dumped ROM to test it out. wrong log, here's [recompiled one](/assets/txt/logs/attempt_ml_2022-11-25/WRONG_COMPILE_WORKS.log)
    - [finally able install Guest Addition](/assets/txt/logs/attempt_ml_2022-11-25/install_guest_addition.log). Oh yeah, will share VHD later.
    - [& vboxguest working](/assets/txt/logs/attempt_ml_2022-11-25/vboxguest_exists_now.log). Damn, I'm sorry I have lost the URL where article tutorial is from.
    - [also pls clone OG ML source code in the VHD](/assets/txt/logs/attempt_ml_2022-11-25/clone_og_ml.log)
    - Moar doing. [log1 BISON LEX](/assets/txt/logs/attempt_ml_2022-11-25/what_did_install_more.log), [log2 try again](/assets/txt/logs/attempt_ml_2022-11-25/go_result_now.log)
    - Hey, Download Ghidra man! https://ghidra-sre.org/ . [Source code](https://github.com/NationalSecurityAgency/ghidra). You haven't been downloading ghidra where you should've the first time hearing it in LiveOverflow YouTube channel.
    - [FORGOR RECONFIGURE WHOOPS](https://www.magiclantern.fm/forum/index.php?topic=20214.msg241033#msg241033 ). also I too late, there's already that Debian WSL. Yeah I get upset easily for compile failures after all these years experiencing so many hard to figure why fails. Don't worry, we'll get both of them cleaned, configured & compiled.
    - here command you should copy. [here](/assets/txt/logs/attempt_ml_2022-11-26/__Copy_Command.txt )
    - [Install Debian WSL](/assets/txt/logs/attempt_ml_2022-11-26/1_install_debian.log). [Download & Info here](https://wiki.debian.org/InstallingDebianOn/Microsoft/Windows/SubsystemForLinux )
    - [Install packages as much as I remember](/assets/txt/logs/attempt_ml_2022-11-26/2_install_packages.log)
    - [Make directories & almost forgot the `vncdotool` pip](/assets/txt/logs/attempt_ml_2022-11-26/3_make_dir_and_pip.log)
    - [Clone ML Simplified & QEMU simplified](/assets/txt/logs/attempt_ml_2022-11-26/3a_try_clone_qemu-eos.log)
    - [Forgot libgtk](/assets/txt/logs/attempt_ml_2022-11-26/5_forgor_gtk.log)
    - [Compile QEMU works too](/assets/txt/logs/attempt_ml_2022-11-26/6_compile_qemu_for_real.log)
    - [Test works sort of idk...](/assets/txt/logs/attempt_ml_2022-11-26/7_finally_test_scripts.log)
    - [Also pls fix the VM Debian ML too](/assets/txt/logs/attempt_ml_2022-11-26/8_fix_vm_compile_too.log)
    - My mom said about 24th December 2022, she will have the final exam of the semester.
    - [Ah damn it, I thought I've already installed `bison` & `flex` what the peck?!](/assets/txt/logs/attempt_ml_2022-11-26/9_ah_man_thought_bison_flex_installed.log)
    - [sigh let's just do it again](/assets/txt/logs/attempt_ml_2022-11-26/10_okay_try_again.log)
    - [install `bison` & `flex` also on the VM](/assets/txt/logs/attempt_ml_2022-11-26/11_bison_flex_vm.log)
    - [Make again VM **WORKS** whew!](/assets/txt/logs/attempt_ml_2022-11-26/12_remake_qemu-eos_vm.log)
...
- Reattempt porting ML.
    ...
    - Now, let's see if QEMU ready. ~~**Awaiting...............**~~ **DONE**
    ...
    - [Prepare Simplified in the VM](/assets/txt/logs/attempt_ml_2022-11-26/12a_prepare_simplified_ml_vm.log)
    - [Forgor Python2 VM](/assets/txt/logs/attempt_ml_2022-11-26/12b_forgor_python2_vm.log)
    - [montage not found VM](/assets/txt/logs/attempt_ml_2022-11-26/15_montage_somehow_not_found_vm.log)
    - [Sigh Synaptic this now! VM](/assets/txt/logs/attempt_ml_2022-11-26/16_lets_synaptic_vm.log)
    - [Oh God! Lots of packages I didn't install! VM](/assets/txt/logs/attempt_ml_2022-11-26/17_damn_lotsof_package_forgor_vm.log)
    - [lots complicated! VM](/assets/txt/logs/attempt_ml_2022-11-26/18_lots_complicated_vm.log)
    - [I'm tired of this! let's just compile 1 even `make docq` skipped! VM **WORKS**](/assets/txt/logs/attempt_ml_2022-11-26/19_gave_up_vm.log)
    - Try & attempt combining. **Always test in the QEMU!**


### Stuffs
- [ccritix's 3000D Platform folder](https://github.com/ccritix/magic-lantern/tree/branches/1300D/platform/4000D.100 ) Complete with Stubs. Lacks the Installation folder atm. OR try using libUSB method? [commit 1](https://github.com/ccritix/magic-lantern/commit/056778668b8d4b8c96432b708d51e2e2ab57f340)
...
- [`eos.c` in ccritix](https://github.com/ccritix/magic-lantern/blob/branches/1300D/contrib/qemu/eos/eos.c) Notice the `4000D` definition! Ctrl + F and take a look for `4000D` Flash ID & stuffs. [commit 1](https://github.com/ccritix/magic-lantern/commit/056778668b8d4b8c96432b708d51e2e2ab57f340 )
- [`eos.c` in names_are_hard](https://github.com/reticulatedpines/qemu-eos/blob/qemu-eos-v4.2.1/hw/eos/eos.c) No 4000D definition, but `1300D` definition! Ctrl + F that too, it's in **Enumeration**. I gotta bring this `4000D` in! [commit 1](https://github.com/reticulatedpines/qemu-eos/commit/f48f1eadb5f660dc1a51ca56f4072cee95f6dcc4 )
